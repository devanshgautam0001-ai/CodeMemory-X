...
describe('DependencyImpactAnalyzer (Reverse Dependency)', () => {
  const analyzer = new DependencyImpactAnalyzer();
  const scorer = new ImpactScorer();

  it('detects REVERSE_DEPENDENCY impact for incoming graph edges', () => {
    ...
    const result = analyzer.analyze('s2', graph, scorer, 1);
    expect(result.nodes.length).toBe(1);
    expect(result.nodes[0].id).toBe('s1');
    expect(result.nodes[0].reasons[0].type).toBe('REVERSE_DEPENDENCY');
  });
});
